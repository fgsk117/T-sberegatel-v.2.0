...
export default function SettingsPage() {
  ...
  useEffect(() => {
    ...
    const loadData = async () => {
      setLoading(true);
      try {
        await Promise.all([loadBlacklist(), loadNotificationSettings()]);
      } finally {
        setLoading(false);
      }
    };
  ...
  const toggleExcludeCategory = (category: string) => {
    setNotifSettings(prev => {
      const newExclude = prev.exclude_categories.includes(category)
        ? prev.exclude_categories.filter(c => c !== category)
        : [...prev.exclude_categories, category];
      return { ...prev, exclude_categories: newExclude };
    });
  };
    ...
    <div className="space-y-6">
      ...
      <div className="bg-zinc-900 rounded-3xl p-6 border border-zinc-800">
        ...
        <div className="space-y-4 mb-6">
          {blacklist.length === 0 && (
            <div className="text-center text-gray-500 py-8">Черный список пуст</div>
          )}
          {blacklist.map(item => (
            <div
              key={item.id}
      ...
      <div className="bg-zinc-900 rounded-3xl p-6 border border-zinc-800">
        ...
        <div className="space-y-6">
          ...
          <div>
            <label className="block text-white font-bold mb-2">Частота уведомлений</label>
            <select
              value={notifSettings.frequency}
              onChange={e =>
                setNotifSettings(prev => ({ ...prev, frequency: e.target.value }))
              }
          ...
          <div>
            <label className="block text-white font-bold mb-2">Канал уведомлений</label>
            <select
              value={notifSettings.channel}
              onChange={e =>
                setNotifSettings(prev => ({ ...prev, channel: e.target.value }))
              }
              className="w-full px-4 py-3 bg-black text-white rounded-2xl border-2 border-zinc-700 focus:border-[#FFDD2D] focus:outline-none"
            >
              ...
              <div className="space-y-2">
                {blacklist.map(item => (
                  ...
                  >
                    <input
                      type="checkbox"
                      checked={notifSettings.exclude_categories.includes(item.category_name)}
