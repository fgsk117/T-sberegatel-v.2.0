...
export default function AdminPanel({ onBack }: { onBack: () => void }) {
  ...
  const updatePrompt = async (id: string, promptText: string) => {
    ...
    try {
      ...
      setPrompts(prev =>
        prev.map(p => (p.id === id ? { ...p, prompt_text: promptText } : p))
      );
            ...
            <div key={prompt.id} className="bg-black rounded-2xl p-6 space-y-4">
              ...
              <div>
                ...
                <textarea
                  ...
                  rows={6}
                  className="w-full px-4 py-3 bg-zinc-900 text-white rounded-2xl border-2 border-zinc-700 focus:border-[#FFDD2D] focus:outline-none font-mono text-sm resize-none"
                />
              </div>
              ...
              >
                ...
                {saving[prompt.id] ? 'Сохраняем...' : 'Сохранить промпт'}
              </button>
            </div>
          ))}
